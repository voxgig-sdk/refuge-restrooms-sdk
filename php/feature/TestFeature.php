...
// RefugeRestrooms SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class RefugeRestroomsTestFeature extends RefugeRestroomsBaseFeature
{
    ...
    public function init(RefugeRestroomsContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (RefugeRestroomsContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = RefugeRestroomsParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (RefugeRestroomsContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(RefugeRestroomsContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = RefugeRestroomsParam::call($ctx, $k);
