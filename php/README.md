# RefugeRestrooms PHP SDK



The PHP SDK for the RefugeRestrooms API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Restroom()` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/refuge-restrooms-sdk/releases](https://github.com/voxgig-sdk/refuge-restrooms-sdk/releases)
...
try {
    ...
    foreach ($restrooms as $record) {
        $item = $record->data_get();
        echo $item["id"] . " " . $item["accessible"] . "\n";
    }
} catch (\Throwable $err) {
...
print_r(array_map(fn($item) => $item->data_get(), $restroom));
```
...
```php
$client = RefugeRestroomsSDK::test($testopts, $sdkopts);
```
...
| `Restroom` | `($data): RestroomEntity` | Create a Restroom entity instance. |
...
```php
// list() returns an array of Restroom records (throws on error).
$restrooms = $client->Restroom()->list();
```
...
```
php/
├── refugerestrooms_sdk.php          -- Main SDK class
...
The main class (`refugerestrooms_sdk.php`) exports the SDK class
...
// $restroom->data_get() now returns the restroom data from the last list
// $restroom->match_get() returns the last match criteria
```
