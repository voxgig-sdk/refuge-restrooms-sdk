...
Provides access to gender-neutral restroom locations and information.

Learn more about Refuge Restrooms API at [refugerestrooms.org](https://refugerestrooms.org).
...
This SDK exposes the API as a small set of **semantic entities** — Restroom — that you
...
const items = await client.Restroom().list()
```
...
```go
client := sdk.Test()
result, err := client.Restroom(nil).List(
    nil, nil,
)
```
...
| --- | --- | --- |
| **Restroom** | The Restroom entity (list). | `/v1/restrooms` |
...
$restrooms = $client->Restroom()->list();
print_r(array_map(fn($item) => $item->data_get(), $restrooms));
```
