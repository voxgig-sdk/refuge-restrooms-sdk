...
require_relative 'RefugeRestrooms_types'
...
class RefugeRestroomsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = RefugeRestroomsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = RefugeRestroomsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, RefugeRestroomsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    RefugeRestroomsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = RefugeRestroomsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = RefugeRestroomsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = RefugeRestroomsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue RefugeRestroomsError => err
    ...
    if fetched.is_a?(Hash)
      status = RefugeRestroomsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = RefugeRestroomsError.new(
        "graphql_error", "RefugeRestroomsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = RefugeRestroomsSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
