...
class RestroomDirectTest < Minitest::Test
  def test_direct_list_restroom
    setup = restroom_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-restroom", setup[:live] ? "live" : "unit")
    if _should_skip
...
def restroom_direct_setup(mockres)
  ...
  if live
    ...
    client = RefugeRestroomsSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
