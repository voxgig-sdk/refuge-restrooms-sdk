...
class RestroomEntity
  ...
  def make
    opts = @_entopts.dup
    RestroomEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = RefugeRestroomsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Restroom, Hash] the current Restroom data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = RefugeRestroomsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Restroom fields)
  def match_get
  ...
  #
  # @param reqmatch [RestroomListMatch, Hash, nil] match filter (any subset of
  #   Restroom fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Restroom>, Array] the matching Restroom items; raises RefugeRestroomsError on failure
