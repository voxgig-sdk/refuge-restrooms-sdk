# refuge-restrooms-mcp

[MCP](https://modelcontextprotocol.io) server exposing the RefugeRestrooms SDK as
two agent tools — `refuge-restrooms_list` and `refuge-restrooms_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/refuge-restrooms-mcp)
make build

# 2. Provide credentials via the environment
export REFUGE_RESTROOMS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user refuge-restrooms \
  -- /absolute/path/to/refuge-restrooms-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./refuge-restrooms-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// refuge-restrooms_list: first page of records
{ "entity": "restroom" }
{ "entity": "restroom", "query": { } }

// refuge-restrooms_load: one record by id
{ "entity": "restroom", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/refuge-restrooms-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user refuge-restrooms \
     -- "$PWD"/dist/*/refuge-restrooms-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `refuge-restrooms_list` and `refuge-restrooms_load` tools now appear
   in new sessions. Ask the agent to *"list restroom using refuge-restrooms"*
   and it calls `refuge-restrooms_list` with `{"entity":"restroom"}`.
...
```sh
export REFUGE_RESTROOMS_APIKEY=sk_live_xxx            # API key
export REFUGE_RESTROOMS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `refuge-restrooms_list` tool
...
### Call the `refuge-restrooms_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `refuge-restrooms_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `refuge-restrooms_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `REFUGE_RESTROOMS_APIKEY` | API key sent with every request. |
| `REFUGE_RESTROOMS_BASE` | Optional override of the API base URL. |
...
```sh
./refuge-restrooms-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"refuge-restrooms_load","arguments":{"entity":"restroom","query":{"id":1}}}}'
```
