...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.REFUGERESTROOMS_TEST_LIVE ||
    'TRUE' === process.env.REFUGERESTROOMS_TEST_OVERRIDE
  ) {
  ...
  m.REFUGERESTROOMS_TEST_EXPLAIN = process.env.REFUGERESTROOMS_TEST_EXPLAIN || m.REFUGERESTROOMS_TEST_EXPLAIN
