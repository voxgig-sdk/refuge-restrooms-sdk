...
//     (\`new RefugeRestroomsSDK(...)\` / \`RefugeRestroomsSDK.test(...)\` become
//     \`RefugeRestroomsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'RefugeRestroomsSDK'
...
const TEST_SEED = {"entity":{"restroom":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new RefugeRestroomsSDK(...)\` and any
// \`RefugeRestroomsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RefugeRestroomsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RefugeRestroomsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
