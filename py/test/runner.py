# RefugeRestrooms SDK test runner

from __future__ import annotations
import os
import json

from refugerestrooms_sdk.utility.voxgig_struct import voxgig_struct as vs


class RefugeRestroomsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RefugeRestroomsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RefugeRestroomsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RefugeRestroomsTestRunner.getenv("REFUGE_RESTROOMS_TEST_LIVE")
        override = RefugeRestroomsTestRunner.getenv("REFUGE_RESTROOMS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RefugeRestroomsTestRunner.getenv(key)
        ...
        explain = RefugeRestroomsTestRunner.getenv("REFUGE_RESTROOMS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if RefugeRestroomsTestRunner._test_control is not None:
            return RefugeRestroomsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RefugeRestroomsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RefugeRestroomsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RefugeRestroomsTestRunner.load_test_control()
...
def load_env_local():
    RefugeRestroomsTestRunner.load_env_local()


def env_override(m):
    return RefugeRestroomsTestRunner.env_override(m)


def entity_data(v):
    return RefugeRestroomsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return RefugeRestroomsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RefugeRestroomsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RefugeRestroomsTestRunner.load_test_control()


def live_client_options():
    return RefugeRestroomsTestRunner.live_client_options()


def live_delay_ms():
    return RefugeRestroomsTestRunner.live_delay_ms()
