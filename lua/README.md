# RefugeRestrooms Lua SDK



The Lua SDK for the RefugeRestrooms API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Restroom()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/refuge-restrooms-sdk/releases)),
...
local result, err = client:Restroom():list()
...
Create an instance: `local restroom = client:Restroom(nil)`
...
| `id` | `number` | Unique identifier for the restroom |
...
```
lua/
├── refuge-restrooms_sdk.lua    -- Main SDK module
...
The main module (`refuge-restrooms_sdk`) exports the SDK constructor
...
```lua
local restroom = client:Restroom()
restroom:list()
