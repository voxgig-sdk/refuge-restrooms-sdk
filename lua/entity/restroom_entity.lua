...
local RestroomEntity = {}
RestroomEntity.__index = RestroomEntity


function RestroomEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RestroomEntity)
  self._name = "restroom"
  self._client = client
...
function RestroomEntity:get_name()
  return self._name
end


function RestroomEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RestroomEntity.new(self._client, opts)
end
...
function RestroomEntity:mark_deleted()
  self._deleted = true
end


function RestroomEntity:deleted()
  return true == self._deleted
end


function RestroomEntity:data_set(args)
  if args ~= nil then
...
function RestroomEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RestroomEntity:match_set(args)
  if args ~= nil then
...
function RestroomEntity:match_get()
...
function RestroomEntity:stream(action, args, callopts)
...
function RestroomEntity:_run_op(ctx, post_done)
  local utility = self._utility
