...
// RefugeRestrooms SDK feature test
//
...
class FtCtrl extends RefugeRestroomsControl
{
...
class FtClient
{
    ...
    private RefugeRestroomsUtility $utility;

    public function __construct(RefugeRestroomsUtility $utility, array $options)
    {
    ...
    public function get_utility(): RefugeRestroomsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RefugeRestroomsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RefugeRestroomsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RefugeRestroomsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RefugeRestroomsConfig::make_feature($name);
    ...
    private static function buildUrl(RefugeRestroomsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RefugeRestroomsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RefugeRestroomsResponse($fetched);
        }
    ...
    private function fail(RefugeRestroomsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RefugeRestroomsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
