...
// RefugeRestrooms SDK retry feature
...
class RefugeRestroomsRetryFeature extends RefugeRestroomsBaseFeature
{
    ...
    public function init(RefugeRestroomsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RefugeRestroomsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RefugeRestroomsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RefugeRestroomsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
