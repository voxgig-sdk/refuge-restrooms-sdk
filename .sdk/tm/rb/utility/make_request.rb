...
module RefugeRestroomsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RefugeRestroomsResponse.new({})
    ...
    if fetch_err
      ...
      response = RefugeRestroomsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RefugeRestroomsResponse.new(fetched)
    else
