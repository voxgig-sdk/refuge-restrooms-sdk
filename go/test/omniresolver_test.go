...
import (
	...
	omni "github.com/voxgig-sdk/refuge-restrooms-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RefugeRestroomsError); is {
				out := map[string]any{
					"name":    "RefugeRestroomsError",
...
func omniCtx(arg any, client *sdk.RefugeRestroomsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RefugeRestroomsSDK); is {
			client = live
