...
func TestRestroomEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Restroom(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Restroom(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set REFUGE_RESTROOMS_TEST_RESTROOM_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		restroomRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.restroom")))
...
func restroomBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("REFUGE_RESTROOMS_TEST_RESTROOM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["REFUGE_RESTROOMS_TEST_RESTROOM_ENTID"])
