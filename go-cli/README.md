# refuge-restrooms-cli

boru-driven command-line client **and** interactive REPL for the RefugeRestrooms
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/refuge-restrooms-cli)
make build

# 2. See usage (words, entities, env vars)
./refuge-restrooms-cli --help

# 3. Provide credentials once, via the environment
export REFUGE_RESTROOMS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./refuge-restrooms-cli list restroom

# 5. Override the API base URL for a single call
REFUGE_RESTROOMS_BASE=https://api.example.com ./refuge-restrooms-cli list restroom

# 6. No arguments -> interactive REPL
./refuge-restrooms-cli
refuge-restrooms> list restroom
refuge-restrooms> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/refuge-restrooms-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/refuge-restrooms-cli list restroom
   ```
...
Run with no arguments to open a REPL (prompt `refuge-restrooms>`). Each line is
evaluated as its own boru expression:

```text
$ ./refuge-restrooms-cli
refuge-restrooms> list restroom
refuge-restrooms> /help
refuge-restrooms> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `restroom`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/refuge-restrooms-cli`. |
...
(`./refuge-restrooms-cli <expr>`) and interactively (the REPL), and expressions compose the
