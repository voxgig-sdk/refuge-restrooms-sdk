// Command RefugeRestrooms-cli is a CLI + REPL for the RefugeRestrooms SDK,
...
const prompt = "refuge-restrooms"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "restroom"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      REFUGE_RESTROOMS_APIKEY (api key), REFUGE_RESTROOMS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: REFUGE_RESTROOMS_APIKEY carries the API key and
	// REFUGE_RESTROOMS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("REFUGE_RESTROOMS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("REFUGE_RESTROOMS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewRefugeRestroomsSDK(opts)
